...
function Coupon() {
  ...
  return (
    <View className="coupon">
      <View className="coupon__tabs">
        <View
          className={tabIndex === 0 ? 'coupon__tab--active' : 'coupon__tab'}
          onClick={() => {
            setTabIndex(0);
            fetchCouponList(0);
          }}
        ...
        <View
          className={tabIndex === 1 ? 'coupon__tab--active' : 'coupon__tab'}
          onClick={() => {
            setTabIndex(1);
            fetchCouponList(1);
          }}
      ...
      <View className="coupon__list">
        {couponList.map((item) => (
          ...
          >
            <View className="coupon__title">
              {item.name}
              <Text className="right" style={{ color: tabIndex === 0 ? '#e80e27' : '#999' }}>
                ￥{item.amount}
              </Text>
            </View>
            <View className="coupon__intro">适用类型：{item.type}</View>
            <View className="coupon__intro">最低消费：{item.minCost}</View>
            <View className="coupon__intro">
              有效期：{item.startDate}
              <Text>至</Text>
              {item.endDate}
            </View>
            <View className="coupon__expired" style={{ display: tabIndex === 0 ? 'none' : 'block' }}>
              已失效
            </View>
          </View>
        ))}
